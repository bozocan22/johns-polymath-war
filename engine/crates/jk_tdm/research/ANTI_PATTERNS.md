...
| the split brain | client re-deriving what the sim computes, then drifting | minigun crosshair, AWM bracket, bow preview v0, BOTH mouse-map labels; **the mech mount keys** — the strip renders from `MechWeapon::for_set` and `cmd.slot` kept a hardcoded copy of the HEAVY's list, so in a medic the number keys selected mounts that chassis does not have and the repair beam was unselectable | shared single-source fns (`base_spread`, `aim_spread_of`, `mouse_map`) + label-matches-binding test + `a_number_key_selects_the_mount_the_strip_labels_with_it` |
| the untouched control | a binding every test satisfies by writing the state the binding is supposed to write | mech mount keys: every mount test set `mech_weapon` directly, so a dead number key passed all of them for as long as it existed | tests that drive `PlayerCmd` through `step` rather than assigning fields; assert against the same list the HUD renders, so a third chassis inherits the guarantee |

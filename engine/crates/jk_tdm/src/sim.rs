...
mod tests {
    ...
    fn a_thirty_shot_ak_spray_is_bit_identical_on_replay() {
        let fire_once = |seed: u64| -> [f32; 2] {
            ...
            s.fighters[0].ammo = 40;
            s.fighters[0].reserve = 0;
            // This measures a SPRAY, so the subject has to survive long
            // enough to finish one. The live enemy is disarmed rather
            // than the assertion loosened: `spray_ramp` made everyone -
            // bots included - straighter over the first rounds, and a
            // sharper bot was killing the shooter around round 29.
            s.fighters[1].ammo = 0;
            s.fighters[1].reserve = 0;
            s.fighters[1].slot_ammo = [(0, 0); 3];
            let mut shots = 0;
